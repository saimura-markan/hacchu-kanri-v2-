...
//     ★例外（2026-09-14・3b）: 見積提出済み→調整中 だけは construction_requested
//       （施工依頼）として送る。トリガー側の WHEN で OLD='見積提出済み' に限定している。
...
const PUSH_TEXT: Record<string, { title: string; body: string }> = {
  ...
  // ★施工依頼（2026-09-14・3b）。見積提出済み→調整中 で飛ぶ。宛先は order_received と同じ
  //   get_push_targets（発注者本人＋社内）。社内がロック画面で新規発注・見積依頼と
  //   見分けられるよう title を分ける。案件情報・開始希望日は載せない（既存と同じ規則）。
  construction_requested: { title: 'E-Li 施工依頼', body: '施工のご依頼を受け付けました' },
};
...
const EVENTS: Record<string, {
  ...
  // 施工依頼（2026-09-14・3b）。AFTER UPDATE OF status 起点
  //   （trg_eli_notify_construction_requested。WHEN OLD='見積提出済み' AND NEW='調整中'）。
  //   ★ 顧客の「施工依頼」ボタンでも、管理者の手動変更（見積提出済み→調整中）でも飛ぶ。
  //     トリガーは変更者を区別しない（3b 決定3）。
  //   ★ expectedStatus は遷移先の '調整中'。発火後すぐ管理者が日程確定などに進めた場合は
  //     status 再確認で skip される（メールも Push も）。その場合は schedule_fixed 側が通知する。
  //   ★ 開始希望日は本文に書かない（案件情報を載せない規則）。リンク先で確認してもらう。
  construction_requested: {
    expectedStatus: '調整中',
    subject: '施工のご依頼を受け付けました',
    heading: '施工のご依頼を受け付けました',
    emoji: '🔨',
    lines: [
      'この度は施工のご依頼をいただき、ありがとうございます。無事に受け付けいたしました。',
      '担当者より作業日程のご提案をいたしますので、今しばらくお待ちください。',
      '内容は下のボタンから E-Li にてご確認いただけます。',
    ],
  },
};
