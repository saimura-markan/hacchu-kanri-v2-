...
### 14-6. ★解決済み：ブラウザでの通知表示（2026-08-15 解決）

> **★2026-08-15 解決。原因は macOS 側の設定で、コードの問題ではなかった。**
> 詳細は本節末尾の「解決記録」を参照。以下は当時の調査記録として残す。
...
**原因：macOS システム設定 → 通知 → Google Chrome の「通知を許可」がオフだった。**

| 項目 | 内容 |
|---|---|
| 対処 | 「通知を許可」を**オン**に変更 |
| あわせて確認 | 通知スタイル **バナー** ／ 集中モード **オフ** |
| 検証 | `new Notification()` で Mac にバナー表示を**確認済み** |
| 結論 | **コード側の問題ではなかった。`sw.js` も送信経路も無罪** |

**★切り分けの見立ては正しかった。** 上の調査で
「Push を使わないローカル通知でも表示されない → 原因は macOS / Chrome の表示層に限定」
まで絞れており、**その範囲の中に答えがあった**。切り分け自体は機能していた。

**★教訓：`registration.showNotification()` や `new Notification()` が
無反応のときは、まず OS 側の通知許可を疑う。**
今回は「表示層に限定される」ところまで絞れていたにもかかわらず、
**OS 設定を実際に見るまでに時間が空いた**。
ブラウザの許可（`Notification.permission === 'granted'`）が通っていても、
**その外側に macOS の許可がもう1段ある**。ブラウザ側が granted でも
OS 側がオフなら何も出ない。両方を見ること。

**★Phase 1 の完了条件を満たした。** 上の「Phase 1 で確認すること」のうち
macOS / Chrome の通知設定は解決済み。**本番 HTTPS 環境での再確認は未実施**なので、
Phase 3 の実機テスト（工程⑥）で兼ねる。

**★これは Chrome デスクトップの話に限られる。** 次節 14-7 のとおり、
**iOS / Safari が通った証明にはならない**。iPhone 実機確認は引き続き必要。
...
### 14-12. iCloud 事故と全復旧・Web Push 現在地の確定（2026-08-14）

**コードは1行も変えていない。** この日はローカル環境の復旧と、
Phase 2 完了後の現在地の確定だけを行った。

#### 何が起きたか

iCloud の容量逼迫でローカルのファイル実体が退避され、
**`~/Documents` 配下のソースが「見えるが中身が無い」状態**になった。
50GB プランへ変更して復旧に着手。

- `~/Library/Mobile Documents` が**存在しない**時間帯があった（11:14 に再作成）
- その間 `~/Documents` は**空に見えていた**。`find ~ -iname '*hoyojo*'` も
  iCloud アーカイブ側の残骸しか拾わず、**「ソースが消えた」と誤判断した**
- 実際にはツリーが未展開だっただけで、ソースは無傷だった

★**同期が動いている最中のディレクトリ一覧は信用できない。**
`ls` が空を返しても消えた証拠にはならない。時間を置いて再確認すること。

#### hoyojo-app の実体化（GitHub に無いので最優先だった）

`~/Documents/hoyojo-app` は**未実体化 340 ファイル**の状態だった。

| 手段 | 結果 |
|---|---|
| `brctl download`（サンドボックス内） | ★**全件失敗。** `brctl: Trying to invoke brctl from a sandboxed process` |
| `brctl download`（サンドボックス外） | exit=0 だが**非同期要求にすぎない**。340 → 265 で停滞し、それ以上進まない |
| **ファイルを実際に読む**（`find ... -exec cat {} +`） | ★**これだけが効いた。** 265 → 0 |

★**iCloud から確実に落とすには `brctl download` ではなく「読む」。**
`brctl` は要求を出すだけで完了を保証しない。`.env*` は除外して流すこと。

実体化の判定は `.icloud` プレースホルダの有無では**足りない**。
`stat -f '%z %b'` で **size>0 かつ blocks=0**（dataless）を数える。

最終状態：app 12 / components 15 / data 1 / lib 6 / private 38 / public 19 / sql 30 = **121 ファイル**、
`CLAUDE.md` 220,869 bytes も実体あり。`node_modules` は除外（`npm install` で再構築）。
`.next` 配下 173 件は未実体化のまま放置（ビルドで再生成されるため取得不要）。

★**サンドボックス内の `find` / `stat` は iCloud 管理下で誤った値を返すことがある。**
実際に「0 ファイル・dataless 0 件」と報告された時点で、
サンドボックス外では 340 件が未実体化だった。**計測はサンドボックス外で行う。**

#### hacchu-kanri の git 破損と復旧

`git status` が `fatal: bad object HEAD` で全く動かなくなっていた。

**破損の実態は「ref に不正な値が入った」ではなく、
push 済みコミット3本分の loose object がローカルから失われ、ref だけが先を指していた**状態。

| ref | 指す先 | 状態 |
|---|---|---|
| `refs/heads/main` | `35321b28` | オブジェクト不在 |
| `refs/remotes/origin/main` | `81eb89b8` | オブジェクト不在 |
| `refs/remotes/origin/HEAD` | `81eb89b8` | オブジェクト不在 |

★`.git/objects/pack/` が**完全に空**で、254 ディレクトリすべてが loose object だった。
pack が無いぶん、同期による個別ファイルの取りこぼしに対して無防備だった。

**復旧手順（この順で通った）**

1. `git update-ref refs/heads/main 4a5dbc4b…` … reflog 先端（実在する commit）へ戻す
2. `git fetch origin` … 欠けていた object を再取得。**壊れた remote ref もこれで直る**
3. `git merge --ff-only origin/main` … 3コミット・498行を回収

`fetch` 時の `failed to store: 100001` は macOS キーチェーンへの保存失敗で、
**取得自体は成功している**（サンドボックス由来。無視してよい）。

★**reflog が唯一の真実だった。** `refs/heads/main` の値は reflog に一度も登場せず、
逆に reflog 先端の `4a5dbc4b` は object として実在した。
ref が壊れたら、まず `.git/logs/HEAD` を読む。

**stash の扱いで踏んだもの**

- 未追跡の `fix_push_select_policy.sql` が、回収する側でも新規追加（`A`）されていた。
  `git stash` は**未追跡ファイルを退避しない**ので、そのままでは `--ff-only` が中断する。
  sha1 が完全一致（`3349c5ad…`）＝内容は同一だったため、scratchpad へ退避してから進めた
- `git stash pop --index` は **index の競合で全体が中断**する（部分適用されない）。
  作業ツリーは無傷のまま `stash@{0}` が保持されるので、実害は無い
- 退避内容は **HEAD に完全に含まれていた**（`index.html` は差分ゼロ、
  docs の差は古い版の2行のみ）。確認のうえ `git stash drop`
  （復元用に `b791bc13…` を控えてから実行）

`git fsck` の error 行はゼロ、`origin/main` と一致。**完全復旧。**

#### ★VAPID 秘密鍵は無事だった（再生成しない）

セッション当初は「前セッションと共に消えた・本番未登録なので再生成する」と考えていたが、
**両方とも誤りだった。**

- §14-11 の記録どおり **Supabase Secrets に3件登録済み**
- **秘密鍵はパスワードアプリに残っていることを確認済み**
- 公開鍵は `index.html:6794` に直書き済み

★**消えたのは scratchpad 上の作業用複製だけ。**
再生成していたら `index.html` の公開鍵差し替えと既存購読の全無効化を
無意味に引き起こしていた。**再生成の前に必ずパスワードアプリを確認すること。**

#### ★Web Push の現在地（コードを読んで確定させた）

| Phase | 状態 | 根拠 |
|---|---|---|
| Phase 0 計測 | **未実施** | §14-1 の数値は概算のまま |
| Phase 1 PWA化 | **完了・デプロイ済み** | manifest / sw.js / アイコン4種 実在 |
| Phase 2 Push土台 | **完了・本番稼働** | テーブル・Secrets・購読UI すべて済み。本番購読1行 |
| Phase 3 送信 | **未着手** | `edge-function-send-order-notification.ts`（8/3付）に `web-push` / `sendNotification` / `VAPID` / `eli_push_subscriptions` の参照が**1件も無い** |
| Phase 4 ポーリング停止 | **未着手** | `setInterval` 7本のまま。**3秒が4本**健在（`poll` 5303 / `pollUnread` 5800 / `fetchNotifications` 10544 / チャット 10572） |

★**§14-11 の見出し「実装完了／未デプロイ」は古い。** 同節の
「本番で起きた不具合と対処」が本番デプロイ済みであることを示している。

#### ★通知が飛ばない核心：`sw.js` に push ハンドラが無い

`sw.js` は34行、リスナーは `install` / `activate` / `fetch`（空）の3つだけ。
**`push` イベントも `showNotification` も存在しない**（13行目に予告があるのみ）。

**現状は「購読はできるが、送っても受け取って表示する側が無い」。**
Phase 3 で Edge Function を実装しても、`sw.js` を直さない限り通知は出ない。

#### 次回（この順序で）

1. ★**`~/Documents` から `~/dev/` へ全リポジトリを退避。**
   iCloud 同期下に `.git` を置き続ける限り同じ破損が再発する。**根本原因は未解決。**
   `mk-daily` / `seed-note` / `hoyojo-app` も同種の object 欠落が無いか `git fsck` で点検する
2. ★**§14-6 の表示問題を解決する。** Push を使わないローカル通知
   （`registration.showNotification()` 直接呼び出し）ですら Mac に表示されなかった。
   **ここが直らないと 3・4 を実装しても通知は見えない。最優先。**
   本番 HTTPS での再確認と macOS / Chrome の通知設定から
3. **`sw.js` に `push` / `notificationclick` ハンドラを追加。**
   §14-6 の教訓どおり通知ごとに `tag` を分ける
   - ★**Phase 3 の Edge Function は `url` を送らず、`id` と `kind` だけ送る。**
     URL は `sw.js` の `parsePushPayload` が `/?order=<id>&kind=<kind>` を組み立てる。
     `raw.url` を送ると、その URL に `&kind=` が付かないため
     **②（タブが無い状態から `openWindow` された経路）が種別を読めず `chat` に倒れ、
     ①（既存タブへの `postMessage` 経路）との非対称が出る**（2026-08-15 に確定）
4. **Edge Function に Push 送信を実装（Phase 3 本体）。**
   `send-order-notification` に相乗り、`npm:web-push@3.6.7` を動的 import、
   service_role で購読を読む、404/410 はその場で DELETE、
   ★**成功時に `last_ok_at` を書かない**（絶対原則に反する）

その後：管理者7名への購読配布（現在1端末のみ）／iPhone 実機確認／
`pushsubscriptionchange` 対応／Phase 0 計測（Phase 4 の効果測定に必要）。

#### 教訓

- **iCloud に `.git` を置かない。** ref・loose object のような小さなファイルが
  取りこぼされ、リポジトリが起動不能になる
- **`brctl download` は完了を保証しない。確実に落とすには読む**
- **サンドボックス内の `find` / `stat` / `brctl` / `pbcopy` は iCloud 相手に信用できない**
  （`pbcopy` の無言失敗は §14-11 に既出。今回 `brctl` と計測系が加わった）
- **「消えた」と判断する前に、同期状態を疑う**
- **記録が自分の記憶に勝った。** VAPID 鍵の件は §14-11 の記述が
  誤った再生成を止めた。実施記録を残す価値はここに出る

#### 今週末の工程（土日2時間ずつ・計4時間）

**ゴール：日曜終了時点で「注文イベントで通知が飛ぶ」を完成させる。**

**土曜（2h）**

| | 内容 | 時間 |
|---|---|---|
| ① | `~/dev/` へ全リポジトリ移動 ＋ `git fsck` 点検（`mk-daily` / `seed-note` / `hoyojo-app`） | 30分 |
| ② | ~~★**§14-6 の表示問題を解決**~~ → **✅ 2026-08-15 解決**（macOS の Chrome 通知許可がオフだった） | 60分 → **実質0分** |
| ③ | `sw.js` に `push` / `notificationclick` ハンドラ着手 | 30分 |

**★②が最短で片づいたので、土曜の残り時間は①と③に充てられる。**
下の「②が最大の不確実要素」という但し書きは**解消済み**。③以降を予定どおり進めてよい。

**★①の `git fsck` 点検は 2026-08-15 に実施済み。**
`hacchu-kanri` / `mk-daily` / `seed-note` / `hoyojo-app` の4リポジトリとも
**error 0件**（dangling commit は rebase/amend の正常な残骸）。
`.icloud` プレースホルダ・dataless ファイルも6リポジトリすべて0件。
**残るは `~/dev/` への移動そのものだけ。**

**★ただし当日、別システム側の対応で割り込みが発生し、①は未完了のまま（2026-08-15）。**
経緯は各プロジェクトの記録を参照。
**Web Push の実装そのものには影響しない**（E-Li のコードは1行も触っていない）。

**日曜（2h）**

| | 内容 | 時間 |
|---|---|---|
| ④ | `sw.js` 仕上げ ＋ ローカルテスト | 40分 |
| ⑤ | **Edge Function に Push 送信を実装**（`send-order-notification` に相乗り／`npm:web-push@3.6.7` を動的 import／service_role で購読読取／404・410 は即 DELETE／★`last_ok_at` は書かない） | 50分 |
| ⑥ | 実機テストで通知点灯を確認 | 30分 |

**来週へ回すもの：** 管理者7名への購読配布／iPhone 実機確認／Phase 4（ポーリング停止）／
★**`badge-72.png`（単色シルエット）を作成**。現在 `sw.js` は `badge` にも `/icon-192.png` を
指定しているが、**Android のステータスバーでは badge がアルファ以外を無視して
白いシルエットに潰される**ため、通常アイコンだとべたっとした白い塊になる。
Chrome デスクトップでは `badge` 自体が無視されるので今週末の検証には影響しない。
**iPhone / Android の実機確認とあわせて詰める**（2026-08-15 に方針1として承認済み）。

**■ ★長期課題（今日は対応しない・2026-08-15 記録）**

**この設計文書を public リポジトリで管理し続けるかを検討する。**
`hacchu-kanri-v2-` は public であり、本ファイルは2,500行超の内部設計文書として
その中にある。鍵や個人情報は含めていないが、**内部テーブル名・Secrets の変数名・
アーキテクチャの詳細は公開されている**。

検討する選択肢は2つ。

1. **E-Li リポジトリ自体を private 化する**
2. **`docs/` を別管理に分離する**（private リポジトリ、または Notion へ移す）

★**今日（2026-08-15）は対応しない。** 実装中に構成を変えると切り分けが難しくなるため、
Web Push が一段落してから判断する。判断を先送りしていること自体をここに残しておく。

★**②が最大の不確実要素。** §14-6 は原因が macOS / Chrome の表示層に絞られている
だけで**特定はできていない**。60分で解決しない場合、③〜⑥を実装しても
⑥の点灯確認は通らない。**②が長引いたら、その日のうちに切り上げて
③以降を先に進める**（送信経路の実装は表示問題と独立して検証できる）。
